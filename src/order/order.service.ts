...
export class OrderService {
  ...
  async paymentMethod(orderId: string, pay: number) {
    const findAndPayment = await this.orderModel
      .findOneAndUpdate(
        { _id: orderId, totalPriceAndDelivery: pay },
        {
    ...
    for (const lineItem of findAndPayment.line_items) {
      await this.productService.decreaseVariantQuantity(
        lineItem.variantId,
        lineItem.quantity,
      );
    }
  ...
  async getTotalPrice(variant: Variant, payload: OrderPayloadDto) {
    const lineItem = payload.line_items.find(
  ...
  async getOrderByVariantId(customerId: string, payload: OrderPayloadDto) {
    ...
    const variantProducts = await this.productService.findVariantByIds(
      payload.line_items.map((item) => item.variantId),
    );
  ...
  async updateOrder(orderId: string, payload: OrderPayloadDto) {
    ...
    return await this.orderModel.findOneAndUpdate(
      { _id: orderId },
      { $set: { line_items: payload.line_items } },
    );
    // return this.orderModel.delete({_id: orderId}).exec()
  }

  async cancelOrder(orderId: ValidateId) {
    return await this.orderModel.findOneAndUpdate(
      { _id: orderId },

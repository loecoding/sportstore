...
export class CustomerService {
  ...
  async addCustomer(createCustomerDto: CreateCustomerDto): Promise<Customer> {
      ...
      const customerId = await this.customerModel.findOne({_id: "61f0de63437080abdb88dd13"})
    ...
    const customer = await this.customerModel.findOne({username: createCustomerDto.username})
    if(customer){
